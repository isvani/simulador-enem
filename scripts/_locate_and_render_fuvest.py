...
a pagina/posicao de questoes especificas da FUVEST 2024 Prova V no PDF
...
Uso:
    python scripts/_locate_and_render_fuvest.py 13 19 26 31 49 57 58 59 60 63 72
"""
import re
import sys
...
def locate_all() -> dict[int, tuple[int, float, float]]:
    ...
    for page_no in range(doc.page_count):
        ...
        for block in d["blocks"]:
            for line in block.get("lines", []):
                ...
                if QUESTION_NUM_RE.match(text) and int(text) == expected_next and expected_next <= TOTAL_QUESTOES:
...
def render(indices: list[int], out_dir: Path) -> None:
    doc = fitz.open(PDF_PATH)
    locations = locate_all()
    ...
    for idx in indices:
        ...
        out_path = out_dir / f"q{idx:02d}_page{page_no + 1}.png"
...
if __name__ == "__main__":
    ...
    out_dir = Path(outdir_arg.split("=", 1)[1]) if outdir_arg else ROOT / "scripts" / "_pdf_cache" / "render"
    indices = [int(a) for a in args]
    render(indices, out_dir)
